...
async function repairAuditIssues(job,issues,reviewImages){
  ...
  for(const [key,value] of Object.entries(repaired)){
    const p=contentPlatform(job,key),brief=contentBrief(job,key,p),media=mediaForPlatform(p,job.media),clean=scrubVisualDescriptions(value,media);
    const errors=validate(p,clean,brief,config(job),{imageCount:media.filter(m=>m.type==='image').length,research:job.research});
