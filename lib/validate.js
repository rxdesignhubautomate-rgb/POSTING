...
export const contentSchema=z.object({caption:z.string().optional(),text:z.string().optional(),title:z.string().optional(),description:z.string().optional(),
  tags:z.array(z.string()).optional(),first_comment_hashtags:z.string().optional(),alt_text:z.string().optional(),alt_texts:z.array(z.string()).optional(),cta:z.literal('LEARN_MORE').optional(),
...
const banned=/\b(guaranteed|100%\s*(?:safe|effective)|clinically proven|cures?|miracle|no side effects|best in india|number one|prescription uplift|doctor[- ]endorsed)\b/i;
...
export function validate(platform,c,brief,cfg,{imageCount=0,research={facts:[]}}={}) {
  ...
  if(platform==='google' && (!/Lucknow/i.test(text)||!/pan-India/i.test(text)))errors.push('Google copy needs Lucknow and pan-India');
  if(platform==='youtube') {
...
export function trimSafely(platform,c,limit) {
  ...
  const paragraphs=text.split(/\n\s*\n/),tail=paragraphs.length>1?paragraphs.pop():'';
