...
import { normalizeChannels,personaFor } from './channels';
import { buildSinglePayload } from './model';
...
export function weeklyBrief(slot,channel,settings){
  ...
  return {
    ...
    notes:[slot.topic.notes,`Persona: ${persona.label}. ${persona.voice}`].filter(Boolean).join('\n'),
...
export async function createWeeklyPlan({week_start,topics,media,settings,seed='rx-week'}){
  const channels=normalizeChannels(settings);
  const plan=planWeek({week_start,topics,media,channels:channels.channels.filter(c=>c.enabled),seed});
  const now=new Date().toISOString(),week={id:randomUUID(),week_start:plan.week_start,topics,status:'queued',stats:{slots:plan.slots.length,shortfalls:plan.shortfalls.length},plan,media,created_at:now,updated_at:now};
  ...
  const jobs=plan.slots.map(slot=>{
    ...
    return {id:randomUUID(),week_id:week.id,channel_id:slot.channel_id,persona:slot.persona,slot_at:slot.slot_at,format:slot.format,media_ids:slot.media_ids,brief:weeklyBrief(slot,channel,channels),media:selectedMedia,content:{},status:'queued',created_at:now,updated_at:now,workspace_id:process.env.PUBLER_WORKSPACE_ID??null};
  });
...
export async function approveWeekJobs(weekId,ids){
  ...
  for(const j of jobs.filter(j=>wanted.has(j.id)))out.push(await withJob(j.id,async(job)=>{if(job.status==='awaiting_approval'||job.status==='ready')job.status='approved';else if(job.status==='needs_fix')throw new Error(`${job.channel_id} needs fixing before approval.`);}));
  return out;
}
async function generateWeekly(job,save){
  ...
  const errors=validate(p,job.content[p],job.brief,{platforms:platformDefaults,env:{IG_HASHTAGS_IN_COMMENT:true}},{imageCount:job.media.filter(m=>m.type==='image').length,research:job.research});
