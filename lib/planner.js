...
import { getProfileMemory } from '../brand-memory/index';
...
function formatFor(channel,media){
  ...
  if(!video&&!image)return ['facebook','linkedin','twitter','threads'].includes(channel.platform)?{format:'text',media:[]}:null;
...
export function planWeek({week_start,topics,media=[],channels=[],seed='rx'}){
  ...
  const slots=[],shortfalls=[],youtubeVideoUse=new Map(),localCities=getProfileMemory('va-lucknow').geo.upCities;
  for(const channel of channels.filter(c=>c.enabled!==false)){
    ...
    for(let i=0;i<max;i++){
      ...
      const time=channel.time_slots?.[i%(channel.time_slots.length||1)]??'10:00',slot_at=istSlot(week_start,i%7,time,seed,slots.length),slotDate=slot_at.slice(0,10);
      const available=media.filter(m=>{
        if(m.posting_date&&m.posting_date!==slotDate)return false;
      ...
      const picked=formatFor(channel,available)??{format:'pending',media:[]};
      ...
      const local=channel.brand==='visualaid_lucknow'||channel.persona==='lucknow_local',geo=local?localCities[(i+(hash(`${seed}:${channel.id}:geo`)[2]%localCities.length))%localCities.length]:null;
      slots.push({id:`${channel.id}-${i+1}`,channel_id:channel.id,platform:channel.platform,label:channel.label,persona:channel.persona,slot_at,topic,format:picked.format,media_ids:ids,status:'queued',...(geo?{geo}:{})});
      made++;lastTopic=topic.id;
    }
    const planned=slots.filter(slot=>slot.channel_id===channel.id),missing=planned.filter(slot=>slot.format==='pending').length;
    if(missing)shortfalls.push({channel_id:channel.id,label:channel.label,needed:missing,filled:max-missing,reason:'media can be added later'});
    else if(made<max&&!shortfalls.some(s=>s.channel_id===channel.id))shortfalls.push({channel_id:channel.id,label:channel.label,needed:max-made,filled:made,reason:'not enough slots'});
  }
