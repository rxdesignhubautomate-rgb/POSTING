...
export function planWeek({week_start,topics,media=[],channels=[],seed='rx'}){
  ...
  for(const channel of channels.filter(c=>c.enabled!==false)){
    ...
    for(let i=0;i<max;i++){
      ...
      slots.push({id:`${channel.id}-${i+1}`,channel_id:channel.id,platform:channel.platform,label:channel.label,persona:channel.persona,slot_at:istSlot(week_start,i%7,time,seed,slots.length),topic,format:picked.format,media_ids:ids,status:'queued',...(geo?{geo}:{})});
