...
export function redact(message){let s=String(message);for(const name of ['ADMIN_PASSWORD','SESSION_SECRET','DATABASE_URL','POSTGRES_URL','BLOB_READ_WRITE_TOKEN','PUBLER_API_KEY','OPENAI_API_KEY'])if(process.env[name])s=s.split(process.env[name]).join('[REDACTED]');return s.replace(/sk-[\w-]+/g,'[REDACTED]');}
