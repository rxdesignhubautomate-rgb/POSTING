...
export function combineValidImports(validRows,existingJobs=[]){
  ...
  return [...groups.values()].map(group=>{
    const first=group.items[0].job,external_id=`rx-daily-${group.day}`,existing=existingByExternal.get(external_id),platforms=[],targets={},boards={},channel_ids=[],importRows=[];
    const notes=[];
    ...
    return {
      ...
      brief:{...first.brief,topic:first.brief.topic,primary_keyword:first.brief.primary_keyword,platforms,notes:notes.join('\n\n---\n\n'),targets,boards},
...
function mediaFitsJob(media,job){
  const wantsVideo=job.media_hint==='video'||VIDEO_FORMATS.has(job.format);
...
export function autoMatchMediaByDate(jobs,pool){
  ...
  for(const [day,dayJobs] of byDate){
    ...
    const needsVideo=empty.some(j=>j.media_hint==='video'||VIDEO_FORMATS.has(j.format));
    const media=available.find(m=>needsVideo?m.type==='video':m.type==='image'||m.cover_for);
