...
function publerFailureDetails(r){return collectFailures(r.payload?.failures??r.payload?.errors??r.failures??r.errors??(r.status==='failed'?r.payload:null));}
...
export async function checkDelivery(job){
  ...
  if(Array.isArray(job.delivery.jobs)&&job.delivery.jobs.length){
    ...
    for(const item of job.delivery.jobs){
      ...
      const foundFailures=publerFailureDetails(r);
