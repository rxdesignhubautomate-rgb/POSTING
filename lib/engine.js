...
import { eligible,validateMediaSet,config,buildPayload } from './model';
import { validate } from './validate';
import { researchStep,generatePlatform,audit } from './ai';
import { publer,jobResult,failures } from './publer';
export function validateAll(job){for(const p of eligible(job)){const errors=validate(p,job.content[p],job.brief,config(job),{imageCount:job.media.filter(m=>m.type==='image').length,research:job.research});if(errors.length)throw new Error(`${p}: ${errors.join('; ')}`);}}
...
export async function checkDelivery(job){
  ...
  if(r.status==='failed'||failures(r.payload?.failures)){job.delivery.status='failed';job.status='needs_attention';job.error=JSON.stringify(r.payload?.failures??r);}
