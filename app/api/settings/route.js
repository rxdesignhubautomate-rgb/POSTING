...
import { settings,saveSettings } from '../../../lib/db';
import { publer } from '../../../lib/publer';
export const runtime='nodejs';export const maxDuration=120;
export async function GET(req){try{authorize(req);return Response.json({...await settings(),configured:{database:Boolean(process.env.DATABASE_URL||process.env.POSTGRES_URL),blob:Boolean(process.env.BLOB_READ_WRITE_TOKEN&&process.env.BLOB_PUBLIC_ORIGIN),publer:Boolean(process.env.PUBLER_API_KEY),workspace:Boolean(process.env.PUBLER_WORKSPACE_ID),anthropic:Boolean(process.env.ANTHROPIC_API_KEY)},workspace_id:process.env.PUBLER_WORKSPACE_ID??''});}catch(e){return errorResponse(e);}}
export async function POST(req){try{
  authorize(req,{write:true});await publer('GET','/me');const ws=await publer('GET','/workspaces');const workspaces=Array.isArray(ws)?ws:ws.workspaces;
