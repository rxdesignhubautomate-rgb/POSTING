...
export async function POST(req){try{
  ...
  return Response.json(await handleUpload({body,request:req,
    onBeforeGenerateToken:async pathname=>{
      authorize(req,{write:true});const match=pathname.match(/^media\/([a-f0-9-]{36})\/[a-zA-Z0-9._-]+\.(?:jpg|jpeg|png|webp|mp4|mov)$/i);if(!match)throw new Error('Invalid upload path.');
