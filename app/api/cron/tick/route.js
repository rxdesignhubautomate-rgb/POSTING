...
export async function POST(req){const started=Date.now(),processed=[],errors=[];let cleanup={skipped:true};try{
  ...
  try{cleanup=await runDailyCleanup();}catch(e){errors.push({id:'retention-cleanup',error:redact(e.message)});}
  ...
  return Response.json({processed,cleanup,errors,duration_ms:Date.now()-started});
