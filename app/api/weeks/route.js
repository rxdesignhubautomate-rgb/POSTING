...
import { defaultChannels,normalizeChannels,isBrand,jobInBrand } from '../../../lib/channels';
...
export async function GET(req){try{
  authorize(req);
  const requested=new URL(req.url).searchParams.get('brand')??'all';if(requested!=='all'&&!isBrand(requested))throw new Error('Invalid brand dashboard.');
  const settings=normalizeChannels(await getSetting('channels',defaultChannels()));
  const weeks=(await Promise.all((await listWeeks()).map(async w=>({...w,jobs:(await listWeekJobs(w.id)).filter(j=>j.status!=='archived'&&jobInBrand(j,requested,settings))})))).filter(w=>requested==='all'||w.jobs.length||w.brand===requested);
  const known=new Set(weeks.map(w=>w.id)),imported=new Map();
  for(const job of (await listJobs(5000)).filter(j=>j.status!=='archived'&&j.source==='calendar_import'&&j.week_id&&!known.has(j.week_id)&&jobInBrand(j,requested,settings))){
...
export async function POST(req){try{
  ...
  const brand=body.brand??'all';if(brand!=='all'&&!isBrand(brand))throw new Error('Invalid brand dashboard.');
  ...
  const settings=normalizeChannels(await getSetting('channels',defaultChannels()));
  const week=await createWeeklyPlan({week_start:body.week_start??new Date().toISOString().slice(0,10),topics,media:body.media??[],settings,seed:body.seed??'rx-live',brand:brand==='all'?null:brand});
